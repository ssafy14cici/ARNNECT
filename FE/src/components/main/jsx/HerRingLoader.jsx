...
export default function HerRingLoader({
  ...
  const ringCoverRef = useRef(null);
  ...
  useEffect(() => {
    ...
    ringCoverRef.current = ringcover;
    ...
    // (선택) size prop이 바뀌면 useEffect가 재실행되므로,
    // 여기서는 window resize에 따른 픽셀 비율만 최소한으로 대응
    const onResize = () => {
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));
    };
