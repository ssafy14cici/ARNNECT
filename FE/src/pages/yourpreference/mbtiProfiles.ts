...
export const MBTI_PROFILES = {
  ANLS: {
    ...
    description: "보이는 것보다 의미의 구조를 먼저 잡는다.\n한 획 한 획이 메시지의 뼈대가 되는 타입.",
  ...
  ANLE: {
    ...
    description: "복잡한 생각을 정돈된 형태로 번역한다.\n현실의 소재도 ‘컨셉’으로 재배치하는 편.",
  ...
  ANCS: {
    ...
    description: "색이 감정이 아니라 서사의 장치다.\n한 팔레트로 분위기 + 의미를 동시에 띄운다.",
  ...
  AMCS: {
    ...
    description: "작품이 하나의 공간/의식처럼 느껴진다.\n감정의 파동을 상징으로 수렴시키는 타입.",
  ...
  RNLE: {
    ...
    description: "꾸미기보다 있는 그대로의 힘을 믿는다.\n차분하지만 오래 보게 만드는 타입.",
  ...
  RNCS: {
    ...
    description: "현실을 출발점으로 삼되, 결과는 아이콘이다.\n강한 대비와 컬러로 서사를 뚜렷하게 만든다.",
  ...
  RMLS: {
    ...
    description: "화려함 대신 정적의 밀도로 승부한다.\n보는 사람이 스스로 의미를 꺼내게 한다.",
  ...
  RMLE: {
    ...
    description: "화면이 깔끔한데 감정이 새어 나온다.\n디테일보다 형태의 리듬을 믿는 타입.",
  ...
  RMCS: {
    ...
    description: "색이 장식이 아니라 빛과 기도처럼 작동한다.\n무드가 깊고 상징이 단단하다.",
...
/**
 * ✅ 매핑이 없어도 페이지를 죽이지 않기 위한 fallback
 * - 프로필이 등록되지 않은 코드가 내려오는 순간, 화면은 표시되고 콘솔로만 경고 남김
 */
export function buildFallbackProfile(code: unknown): MbtiProfile {
  const key = normalizeCode(code) || "UNKNOWN";
  return {
    title: "프로필 준비 중",
    tagline: `MBTI 코드(${key})에 대한 설명이 아직 등록되지 않았습니다.`,
    description:
      `서버에서 "${key}" 타입이 내려왔지만 MBTI_PROFILES에 해당 엔트리가 없어요.\n` +
      `mbtiProfiles.ts에 "${key}" 키로 프로필을 추가하면 정상 노출됩니다.`,
    strengths: ["—"],
    watchouts: ["—"],
    tip: "콘솔에서 mbtiCode를 확인하고 프로필을 등록하세요.",
  };
}
